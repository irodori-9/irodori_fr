...
function HideGlobalFooter() {
  useEffect(() => {
    const selectors = ["#app-footer","[data-bottom-nav]","[data-role='bottom-nav']",".bottom-nav",".mobile-tabbar","footer"]
...
type ExecuteResponse = {
  ...
  executions: Array<{ rule_id: number; action_id: number; action_type: string; tanabota_amount: number }>
}
...
export default function POSApp() {
  ...
  const [scanSession, setScanSession] = useState(0) // ★開くたびに増やして <QrScanner key={...}/> を再マウント
  ...
  const handlePayment = async () => {
    if (!scannedUserId || !amount) { setError("ユーザーIDと金額を入力してください"); return }
    if (isNaN(Number(amount)) || Number(amount) <= 0) { setError("正しい金額を入力してください"); return }
    if (!baseUrl) { setError("APIのベースURLが設定されていません（NEXT_PUBLIC_API_BASE_URL）"); return }

    setIsProcessing(true); setError("")
    try {
      const url = joinUrl(baseUrl, "/pos/execute")
      setLastTriedUrl(url)
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: scannedUserId, amount: Number(amount) }),
      })
      if (!res.ok) {
        const maybeJson = await res.json().catch(() => ({ detail: `HTTP ${res.status}` }))
        throw new Error((maybeJson.detail || maybeJson.message || `支払い処理に失敗しました (HTTP ${res.status})`))
      }
      const data: ExecuteResponse = await res.json()
      console.log("ExecuteResponse:", data)
      setPaymentComplete(true)
      setTimeout(() => resetAfterPayment(false), 1500)
    } catch (err: any) {
  ...
  const resetForm = () => { setScannedUserId(""); setAmount(""); setError(""); setPaymentComplete(false) }
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-5 shadow-xl">
          ...
          <AnimatePresence>
            {paymentComplete && (
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
                className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
                <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white rounded-3xl p-8 text-center max-w-sm w-full">
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.05, type: "spring", stiffness: 200 }}
          ...
          {scannedUserId && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-green-700 text-sm text-center"><span className="font-medium">ユーザーID:</span> {scannedUserId}</p>
            </motion.div>
          )}
          ...
          <div className="mb-5">
            ...
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-lg font-semibold">¥</div>
              <input
                type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="金額を入力してください"
                ...
                disabled={isProcessing} inputMode="numeric" min={0}
              />
            </div>
          </div>
          ...
          {(scannedUserId || amount) && !isProcessing && (
            <button onClick={resetForm} className="w-full mt-4 bg-gray-200 text-gray-700 font-medium py-3 rounded-2xl hover:bg-gray-300 transition-colors">
              リセット
            </button>
          )}
        </div>
