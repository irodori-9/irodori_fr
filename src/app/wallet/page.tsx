...
import { CreditCard, ChevronDown, QrCode } from "lucide-react"
...
export default function WalletPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === "card" ? <CardView userId={userIdValue} /> : <DetailsView />}
    </div>
  )
}

const CardView = ({ userId }: { userId: string }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="relative p-6 bg-gradient-to-br from-purple-400/50 to-fuchsia-400/30 backdrop-blur-xl rounded-3xl border border-white/40 shadow-xl overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between mb-6">
            <p className="font-mono text-2xl tracking-widest text-white">4111 1111 1111 1111</p>
            <button
              type="button"
              aria-label="Show QR Code"
              className="inline-flex items-center justify-center rounded-lg p-1.5 bg-white/90 hover:bg-white text-purple-700 shadow-sm"
      ...
      <Dialog open={qrOpen} onOpenChange={setQrOpen}>
        <DialogContent className="bg-white/95 p-4 sm:p-6 border-none max-w-xs sm:max-w-sm" showCloseButton>
          <div className="flex flex-col items-center justify-center">
            <div className="bg-white p-3 rounded-md shadow">
              <QRCodeCanvas value={userId || ""} size={240} includeMargin={true} />
            </div>
          </div>
        </DialogContent>
      </Dialog>
